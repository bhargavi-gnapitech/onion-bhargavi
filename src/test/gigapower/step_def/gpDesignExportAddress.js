  ...
  When('the user clicks on the Export button',{ timeout: 60000 }, async function () {
  ...
  Then('the app displays the Export Design dialog',{ timeout: 60000 }, async function () {
  ...
  Then('then the user clicks the Address button',{ timeout: 60000 }, async function () {

    await regression.clickExportAddress();
   
  });



  Then('then the app downloads the export file to the Downloads folder',{ timeout: 60000 }, async function () {

    
    await regression.handleDialog('Export test_234', 'Close');
